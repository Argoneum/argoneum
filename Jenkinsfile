...
// which are part of the Argoneum repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="argoneum-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /argoneum-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "argoneum-argoneum/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from argoneum-argoneum/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'argoneum-argoneum/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from argoneum-argoneum/develop"
            }
          }

          if (hasCache) {
            sh "cd /argoneum-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /argoneum-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /argoneum-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /argoneum-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /argoneum-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /argoneum-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /argoneum-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
