...
# argoneum_hash
RUN git clone https://github.com/argoneum/argoneum_hash
RUN cd argoneum_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} argoneum
RUN useradd -u ${USER_ID} -g argoneum -s /bin/bash -m -d /argoneum argoneum
...
RUN mkdir /argoneum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /argoneum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /argoneum-src
