...
const char * const BITCOIN_CONF_FILENAME = "argoneum.conf";
const char * const BITCOIN_PID_FILENAME = "argoneumd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "argoneum" is a composite category enabling all Argoneum-related debug output
                if(ptrCategory->count(std::string("argoneum"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Argoneum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Argoneum
    // Mac: ~/Library/Application Support/Argoneum
    // Unix: ~/.argoneum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Argoneum";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Argoneum";
#else
    // Unix
    return pathRet / ".argoneum";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty argoneum.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override argoneum.conf
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Bitcoin Core or Dash Core copyright is not removed by accident
