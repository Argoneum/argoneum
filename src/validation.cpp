...
// Copyright (c) 2014-2019 The Argoneum Core developers
...
#if defined(NDEBUG)
# error "Argoneum Core cannot be compiled without assertions."
#endif
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Argoneum
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // Original Argoneum reward calculation
    if (nPrevHeight == 0)
        nSubsidy = 1240000 * COIN;
    else if (nPrevHeight + 1 > 10000)
        nSubsidy = 30 * COIN;
    else if (nPrevHeight + 1 > 3000)
        nSubsidy = 15 * COIN;
    else
        nSubsidy = 0.1 * COIN;
    ...
    // Updated Argoneum reward table
    if (nPrevHeight + 1 > 37843200)
        nSubsidy = 0.2 * COIN;
    else if (nPrevHeight + 1 > 32587200)
        nSubsidy = 0.5 * COIN;
    else if (nPrevHeight + 1 > 22075200)
        nSubsidy = 1 * COIN;
    else if (nPrevHeight + 1 > 11563200)
        nSubsidy = 2 * COIN;
    else if (nPrevHeight + 1 > 6307200)
        nSubsidy = 3 * COIN;
    else if (nPrevHeight + 1 > 3679200)
        nSubsidy = 4 * COIN;
    else if (nPrevHeight + 1 > 1051200)
        nSubsidy = 5 * COIN;
    else if (nPrevHeight + 1 > 555840)
        nSubsidy = 15 * COIN;
...
CAmount GetFounderPayment(int nHeight, CAmount blockValue) {
    return ((nHeight > 1) && (nHeight <= 555840)) ? (blockValue * Params().FounderFee()) : 0;
}
...
void ThreadScriptCheck() {
    RenameThread("argoneum-scriptch");
...
static int64_t nTimeArgoneumSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // ARGONEUM : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(ARGONEUM): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(AGM): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(AGM): spork is off, skipping transaction locking checks\n");
    }
    ...
    // ARGONEUM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ARGONEUM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ARGONEUM): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(AGM): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeArgoneumSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - Argoneum specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeArgoneumSpecific * 0.000001);
    ...
    if (founderPayment > 0) {
        ...
        if (!FounderPaid) {
            return state.DoS(0, error("ConnectBlock(ARGONEUM): no founder reward"), REJECT_INVALID, "no-founder-reward");
        }
    }
